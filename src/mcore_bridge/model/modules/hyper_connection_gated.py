...
class Qwen4ExpTextGatedResidual(nn.Module):
    def __init__(self, config, use_combine: bool = True):
        ...
        self.hc_norm = Qwen4ExpTextGroupedRMSNorm(hc_hidden_size, group_size=self.hidden_size,
                                                  eps=config.layernorm_epsilon, dtype=config.params_dtype)
        self.input_mix_weight_down = nn.Linear(hc_hidden_size, config.hc_lowrank,
                                               bias=False, dtype=config.params_dtype)
        self.input_mix_weight_up = nn.Linear(config.hc_lowrank, hc_hidden_size,
                                             bias=False, dtype=config.params_dtype)
        self.block_inject_weight = nn.Linear(hc_hidden_size, self.hc_count, bias=False,
                                             dtype=config.params_dtype) if use_combine else None
