    ...
    def _set_ple_ngram_embedding(self, ple, hf_state_dict, to_mcore: bool, pp_src_rank: int):
        ...
        if to_mcore:
            ...
            if self._PLE_NGRAM_SCALE_KEY in hf_state_dict:
                ...
                logger.warning(f'`{self._PLE_NGRAM_SCALE_KEY}` not found in the checkpoint; assuming the PLE ngram '
            ...
            if scale is None:
                logger.warning(f'`{self._PLE_NGRAM_SCALE_KEY}` was not seen during loading; exporting the PLE ngram '
